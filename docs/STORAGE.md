...
0.2.0 使用 Node 内置 SQLite（单个 Registry 实例、WAL、外键约束、事务、busy timeout）。`src/store.js` 是 SQL Repository 边界；HTTP 权限和业务流程在 `src/app.js`，更换数据库时可实现同一 Repository 操作，不需更改 Hub API 或重写 OAuth／业务流程。
