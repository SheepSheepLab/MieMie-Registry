...
export function openStore(path) {
  ...
  if (schema > 1) throw new Error('数据库版本高于当前程序；请使用较新程序');
  ...
  function entryDTO(row, privateView = false) {
    ...
    if (privateView) Object.assign(dto, { status: row.owner_status, moderation: row.moderation, moderationReason: row.moderation_reason });
    return dto;
  }
  ...
  const sourceDuplicate = (url, except='') => db.prepare('SELECT id FROM submissions WHERE source_url=? AND id<>?').get(url,except);
  ...
  function listCatalog({page,pageSize,source,q}) {
  ...
  function insertSubmission({id,ownerId,input,github,time}) {
    db.prepare('INSERT INTO submissions(id,owner_id,name,description,author,source_type,source_url,icon,tags_json,github_json,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)').run(id,ownerId,input.name,input.description,input.author,input.sourceType,input.sourceUrl,input.icon,JSON.stringify(input.tags),github?JSON.stringify(github):null,time,time);
  }
  function updateSubmission({id,input,github,time}) {
    db.prepare('UPDATE submissions SET name=?,description=?,author=?,source_type=?,source_url=?,icon=?,tags_json=?,github_json=?,updated_at=? WHERE id=?').run(input.name,input.description,input.author,input.sourceType,input.sourceUrl,input.icon,JSON.stringify(input.tags),github?JSON.stringify(github):null,time,id);
  }
