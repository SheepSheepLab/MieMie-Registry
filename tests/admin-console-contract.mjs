...
const click=async label=>{const b=[...w.document.querySelectorAll('button')].find(x=>x.textContent===label);assert.ok(b,label);b.click();await new Promise(r=>setTimeout(r,40));};
try{
 w.eval(adminScript);
 ...
 await check('Owner Console hides and recovers without content editor',async()=>{const reason=w.document.querySelector('article input');reason.value='Fixture hide';await click('Hide');await until(()=>f.store.getEntry(row.id).moderation==='hidden');w.document.querySelector('article input').value='Fixture recover';await click('Recover');await until(()=>f.store.getEntry(row.id).moderation==='visible');assert.equal(f.store.getEntry(row.id).name,row.name);});
 await check('Owner protection and Hold controls work',async()=>{w.document.querySelector('article input').value='Fixture hold';await click('设置Security Hold');await until(()=>f.store.getEntry(row.id).security_hold===1);});
 ...
 await check('Owner roles list stays in independent Console',async()=>{await click('角色与封禁');assert.ok(w.document.body.textContent.includes('授予 official_publisher'));});
 await check('Logout clears Console data; Moderator UI has only Community actions',async()=>{await click('退出登录');who='ADMIN';await click('使用 Discord 登录');await until(()=>w.document.body.textContent.includes('Development Fixture ADMIN'));assert.ok(!w.document.body.textContent.includes('角色与封禁'));assert.ok(!w.document.body.textContent.includes('Security Hold'));assert.ok(w.document.body.textContent.includes('Hide'));});
